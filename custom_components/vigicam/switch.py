...
SWITCHES: tuple[VIGISwitchDescription, ...] = (
    ...
    VIGISwitchDescription(
        ...
        openapi_turn_on_fn=lambda oapi: oapi.set_people_detection_switch(True),
        openapi_turn_off_fn=lambda oapi: oapi.set_people_detection_switch(False),
    ),
    VIGISwitchDescription(
        ...
        openapi_turn_on_fn=lambda oapi: oapi.set_vehicle_detection_switch(True),
        openapi_turn_off_fn=lambda oapi: oapi.set_vehicle_detection_switch(False),
    ),
    VIGISwitchDescription(
        key="crossline_detection",
        name="Detection CrossLine",
        icon="mdi:vector-line",
        is_on_fn=lambda d: d.get("openapi_crossline", {}).get("enabled") == "on",
        turn_on_fn=lambda api: None,  # only via OpenAPI
        turn_off_fn=lambda api: None,
        supported_fn=lambda d: bool(d.get("openapi_crossline")),
        openapi_turn_on_fn=lambda oapi: oapi.set_crossline_detection_switch(True),
        openapi_turn_off_fn=lambda oapi: oapi.set_crossline_detection_switch(False),
    ),
    VIGISwitchDescription(
        key="invasion_detection",
        name="Detection Invasion",
        icon="mdi:motion-sensor",
        is_on_fn=lambda d: d.get("openapi_invasion", {}).get("enabled") == "on",
        turn_on_fn=lambda api: None,
        turn_off_fn=lambda api: None,
        supported_fn=lambda d: bool(d.get("openapi_invasion")),
        openapi_turn_on_fn=lambda oapi: oapi.set_invasion_detection_switch(True),
        openapi_turn_off_fn=lambda oapi: oapi.set_invasion_detection_switch(False),
    ),
    VIGISwitchDescription(
        key="area_entry_detection",
        name="Detection Area Entry",
        icon="mdi:map-marker-plus",
        is_on_fn=lambda d: d.get("openapi_area_entry", {}).get("enabled") == "on",
        turn_on_fn=lambda api: None,
        turn_off_fn=lambda api: None,
        supported_fn=lambda d: bool(d.get("openapi_area_entry")),
        openapi_turn_on_fn=lambda oapi: oapi.set_area_entry_detection_switch(True),
        openapi_turn_off_fn=lambda oapi: oapi.set_area_entry_detection_switch(False),
    ),
    VIGISwitchDescription(
        key="area_leave_detection",
        name="Detection Area Leave",
        icon="mdi:map-marker-minus",
        is_on_fn=lambda d: d.get("openapi_area_leave", {}).get("enabled") == "on",
        turn_on_fn=lambda api: None,
        turn_off_fn=lambda api: None,
        supported_fn=lambda d: bool(d.get("openapi_area_leave")),
        openapi_turn_on_fn=lambda oapi: oapi.set_area_leave_detection_switch(True),
        openapi_turn_off_fn=lambda oapi: oapi.set_area_leave_detection_switch(False),
    ),
    VIGISwitchDescription(
        key="drop_take_detection",
        name="Detection Object Drop/Take",
        icon="mdi:package-variant",
        is_on_fn=lambda d: d.get("openapi_drop_take", {}).get("enabled") == "on",
        turn_on_fn=lambda api: None,
        turn_off_fn=lambda api: None,
        supported_fn=lambda d: bool(d.get("openapi_drop_take")),
        openapi_turn_on_fn=lambda oapi: oapi.set_drop_and_take_detection_switch(True),
        openapi_turn_off_fn=lambda oapi: oapi.set_drop_and_take_detection_switch(False),
    ),
    VIGISwitchDescription(
        key="loiter_detection",
        name="Detection Loitering",
        icon="mdi:account-clock",
        is_on_fn=lambda d: d.get("openapi_loiter", {}).get("enabled") == "on",
        turn_on_fn=lambda api: None,
        turn_off_fn=lambda api: None,
        supported_fn=lambda d: bool(d.get("openapi_loiter")),
        openapi_turn_on_fn=lambda oapi: oapi.set_loiter_detection_switch(True),
        openapi_turn_off_fn=lambda oapi: oapi.set_loiter_detection_switch(False),
    ),
    VIGISwitchDescription(
        key="scene_change_detection",
        name="Detection Scene Change",
        icon="mdi:image-broken-variant",
        is_on_fn=lambda d: d.get("openapi_scene_change", {}).get("enabled") == "on",
        turn_on_fn=lambda api: None,
        turn_off_fn=lambda api: None,
        supported_fn=lambda d: bool(d.get("openapi_scene_change")),
        openapi_turn_on_fn=lambda oapi: oapi.set_scene_change_detection_switch(True),
        openapi_turn_off_fn=lambda oapi: oapi.set_scene_change_detection_switch(False),
    ),
    VIGISwitchDescription(
        key="audio_anomaly_detection",
        name="Detection Audio Anomaly",
        icon="mdi:waveform",
        is_on_fn=lambda d: d.get("openapi_audio_anomaly", {}).get("enabled") == "on",
        turn_on_fn=lambda api: None,
        turn_off_fn=lambda api: None,
        supported_fn=lambda d: bool(d.get("openapi_audio_anomaly")),
        openapi_turn_on_fn=lambda oapi: oapi.set_audio_anomaly_detection_switch(True),
        openapi_turn_off_fn=lambda oapi: oapi.set_audio_anomaly_detection_switch(False),
    ),
    VIGISwitchDescription(
